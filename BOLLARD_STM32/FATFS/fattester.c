...
//写SD卡文件
void write_file(u8 *buf,u32 index,u32 size,char *type)
{
	char filename[30]={"0://"};
	char filepath[30]={"0://"};
	...
	hextoascii(buf+size,calendar.w_year%256);
	hextoascii(buf+size+2,calendar.w_year/256);
	hextoascii(buf+size+4,calendar.w_month);
	hextoascii(buf+size+6,calendar.w_date);
	hextoascii(buf+size+8,calendar.hour);
	hextoascii(buf+size+10,calendar.min);
	hextoascii(buf+size+12,calendar.sec);
...
//读SD卡文件
void read_file(u8 *buf,u32 index,u32 size,char *type)
{
	char filename[30]={"0://"};
	char filepath[30]={"0://"};
	strcat(filename,type);
	...
	f_open(file,filename,FA_READ | FA_OPEN_ALWAYS | FA_OPEN_EXISTING);
	f_lseek(file,index*size);
